...
/**
 * The same two ideas, applied to a customer's review clip.
 *
 * Separate from the hero pair rather than parameterised, because the sizes come
 * from different reasoning and would otherwise drift into one compromise that
 * suits neither. A hero fills the viewport; a review clip sits in a card a few
 * hundred pixels wide and is opened deliberately, one at a time.
 *
 * The width cap matters MORE here than in the hero, not less: review video is
 * uploaded straight off a phone, so the master is whatever that phone shot —
 * frequently 4K — and there may be one per review on a page showing fifty.
 */
const REVIEW_MAX_WIDTH = 720;

const REVIEW_VIDEO_TRANSFORM = [
  "f_auto",
  "q_auto",
  "vc_auto",
  `w_${REVIEW_MAX_WIDTH}`,
...
/** Delivery variant of a review clip — transcoded, capped at 720px wide. */
export function reviewVideoUrl(src: string): string {
  const match = CLOUDINARY_VIDEO_UPLOAD.exec(src);
  if (!match) return src;
  const [, base, rest] = match;
  return `${base}${REVIEW_VIDEO_TRANSFORM}/${rest}`;
}

/**
 * First-frame still for a review clip's `poster`. Same extension-replacement
 * caveat as heroVideoPosterUrl — see the note there.
 */
export function reviewVideoPosterUrl(src: string): string | undefined {
  const match = CLOUDINARY_VIDEO_UPLOAD.exec(src);
  if (!match) return undefined;
  const [, base, rest] = match;
  const stem = rest.replace(/\.(mp4|webm|mov|ogg|m4v)$/i, "");
  return `${base}${REVIEW_POSTER_TRANSFORM}/${stem}.jpg`;
}
